...
import net.takie.takieterra.block.entity.inventory.CookingPotInventory;
import net.takie.takieterra.block.entity.inventory.RecipeWrapper;
import net.takie.takieterra.mixin.RecipeManagerAccessor;
import net.takie.takieterra.particle.TakieTerraParticles;
import net.takie.takieterra.recipe.CookingPotRecipe;
import net.takie.takieterra.recipe.TTRecipeType;
import net.takie.takieterra.screen.CookingPotScreenHandler;
...
public class CookingPotBlockEntity extends SyncedBlockEntity implements CookingPotInventory, HeatableBlockEntity, ExtendedScreenHandlerFactory, Nameable {
    ...
    public CookingPotBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(TTBlockEntityType.COOKING_POT.get(), blockPos, blockState);
    ...
    @Override
    public Text getName() {
        return customName != null ? customName : TakieTerraMod.i18n("container.cooking_pot");
    }
    ...
    private Optional<CookingPotRecipe> getMatchingRecipe(RecipeWrapper inventory) {
        ...
        if (lastRecipeID != null) {
            Recipe<Inventory> recipe = ((RecipeManagerAccessor) world.getRecipeManager())
                    .getAllForType(TTRecipeType.COOKING_RECIPE_SERIALIZER.type())
                    .get(lastRecipeID);
        ...
        if (checkNewRecipe) {
            Optional<CookingPotRecipe> recipe = world.getRecipeManager().getFirstMatch(TTRecipeType.COOKING_RECIPE_SERIALIZER.type(), inventory, world);
            if (recipe.isPresent()) {
    ...
    public static void animationTick(World world, BlockPos pos, BlockState state, CookingPotBlockEntity cookingPot) {
        if (world != null && cookingPot.isHeated(world, pos)) {
            ...
            if (random.nextFloat() < .05f) {
                ...
                world.addParticle(TakieTerraParticles.STEAM_PARTICLE.getType(), baseX, baseY, baseZ, .0d, motionY, .0d);
            }
        }
    }
