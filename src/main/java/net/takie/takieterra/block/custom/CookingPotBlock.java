...
import net.takie.takieterra.util.CompoundTagUtils;
import net.takie.takieterra.util.MathUtils;
...
public class CookingPotBlock extends BlockWithEntity implements InventoryProvider, Waterloggable {
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return TTBlockEntityType.COOKING_POT.get().instantiate(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world.isClient()) {
            return checkType(type, TTBlockEntityType.COOKING_POT.get(), CookingPotBlockEntity::animationTick);
        } else {
            return checkType(type, TTBlockEntityType.COOKING_POT.get(), CookingPotBlockEntity::cookingTick);
        }
    }
            ...
            if (inventoryTag.contains(CompoundTagUtils.TAG_KEY_ITEM_LIST, CompoundTagUtils.TAG_LIST)) {
                ...
                if (!meal.isEmpty()) {
                    MutableText servingsOf = meal.getCount() == 1
                            ? TakieTerraMod.i18n("tooltip.cooking_pot.single_serving")
                            : TakieTerraMod.i18n("tooltip.cooking_pot.many_servings", meal.getCount());
                    ...
                    tooltip.add(TakieTerraMod.i18n("tooltip.cooking_pot.empty").formatted(Formatting.GRAY));
                }
            }
        } else {
            tooltip.add(TakieTerraMod.i18n("tooltip.cooking_pot.empty").formatted(Formatting.GRAY));
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (blockEntity instanceof CookingPotBlockEntity cookingPotBlockEntity && cookingPotBlockEntity.isHeated()) {
            SoundEvent boilSound = !cookingPotBlockEntity.getMeal().isEmpty() ? TTSoundEvents.BLOCK_COOKING_POT_BOIL_SOUP.get()
                    : TTSoundEvents.BLOCK_COOKING_POT_BOIL.get();
    ...
    private CookingPotSupport getTrayState(WorldAccess world, BlockPos pos) {
        return world.getBlockState(pos.down()).isIn(TakieTerraTags.Blocks.TRAY_HEAT_SOURCES) ? CookingPotSupport.TRAY : CookingPotSupport.NONE;
    }
