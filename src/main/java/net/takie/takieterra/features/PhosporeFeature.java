...
public class PhosporeFeature extends HugeMushroomFeature {
    public PhosporeFeature(Codec <HugeMushroomFeatureConfig> codec) {
        super(codec);
    }
    ...
    protected boolean canGenerate(WorldAccess world, BlockPos pos, int height, BlockPos.Mutable mutablePos, HugeMushroomFeatureConfig config) {
        ...
        if(i >= world.getBottomY() + 1 && i + height + 1 < world.getTopY()) {
            ...
            if(!isSoil(blockState) && !blockState.isIn(TakieTerraTags.Blocks.BRILLIOME_GROW_BLOCK)) {
                return false;
            }
            else {
