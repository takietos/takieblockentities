...
import net.takie.takieterra.block.custom.LotusStem;
...
public class LotusPlantFeature extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if(structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER) || structureWorldAccess.isAir(blockPos2)) {
            BlockState blockState = TakieTerraBlocks.LOTUS_FLOWER.getDefaultState();
            BlockState blockState2 = TakieTerraBlocks.LOTUS_STEM.getDefaultState().with(LotusStem.WATERLOGGED, structureWorldAccess.getFluidState(blockPos2).isIn(FluidTags.WATER));
            ...
            for(int l = 0; l <= k; ++l) {
                if(structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER) && blockState2.canPlaceAt(structureWorldAccess, blockPos2)) {
                    ...
                    if(blockState.canPlaceAt(structureWorldAccess, blockPos2) && structureWorldAccess.getBlockState(blockPos2.down()).isOf(TakieTerraBlocks.LOTUS_STEM)) {
