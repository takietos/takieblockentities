...
public class PaloVerdeTrunkPlacer extends TrunkPlacer {
	...
	protected TrunkPlacerType <?> getType() {
		return TakieTerraTrunkPlacerTypes.PALO_VERDE_TRUNK_PLACER;
	}
	...
	public List <FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer <BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
		...
		for(int i = 0; i < height; ++i) {
			...
			if (bl && i >= 2) {
				Direction forkedDirection = Direction.Type.HORIZONTAL.random(random);

				BlockPos.Mutable forkedMutablePos1 = mutable2.set(startPos.getX(), j, startPos.getZ());
				...
				if(i == height - 2) {
					list.add(new FoliagePlacer.TreeNode(forkedBlockPos.up(1), 0, false));
				}
			}
		}
	...
	private void generateExtraBranch(TestableWorld world, BiConsumer <BlockPos, BlockState> replacer, Random random, TreeFeatureConfig config, List <FoliagePlacer.TreeNode> nodes, BlockPos pos, int height, int yOffset, Direction direction, int length, int steps) {
